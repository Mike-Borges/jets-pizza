...
export default function MenuCardCustomize({ name, image, desc, type, onClose }: MenuCardCustomizeProps) {
  ...
  const getNumericPrice = (): number => {
    ...
    if (type === 'drink') return selectedDrinkSize ? selectedDrinkSize.price : 0;
    return 0;
  };
  ...
  const handleAddToCart = () => {
    addItem({
      ...
      type,
      selections: {
        size: selectedSize?.name ?? selectedDrinkSize?.name,
        crust: selectedCrust?.name,
        sauce: selectedSauce ?? selectedVeganSauce ?? selectedWingsSauce ?? selectedDipping ?? selectedDressing ?? selectedCalzoneDressing ?? undefined,
        meats: selectedMeats.length > 0 ? selectedMeats : selectedBreadToppings.length > 0 ? selectedBreadToppings : selectedCalzoneToppings.length > 0 ? selectedCalzoneToppings : undefined,
        veggies: selectedVeggies.length > 0 ? selectedVeggies : selectedVeganVeggies.length > 0 ? selectedVeganVeggies : undefined,
        cheese: selectedCheese.length > 0 ? selectedCheese : selectedVeganCheese.length > 0 ? selectedVeganCheese : selectedBreadCheese.length > 0 ? selectedBreadCheese : selectedCalzoneCheese.length > 0 ? selectedCalzoneCheese : undefined,
        bake: selectedBake ?? undefined,
        turbo: selectedTurbo ?? undefined,
      },
    });
    onClose();
  };
